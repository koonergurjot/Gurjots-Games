...
.pong-canvas-wrap{
  ...
  width:100%; height:100%; max-width:100%; padding:10px; margin:0 auto;
}
.pong-canvas{
  width:100%; max-width:1100px; aspect-ratio:16/9;
  touch-action:none; background: transparent; border-radius:12px;
...
.theme-crt .pong-canvas-wrap{position:relative; overflow:hidden;}
.theme-crt .pong-canvas-wrap::after{content:""; position:absolute; inset:0; background:repeating-linear-gradient(to bottom, rgba(255,255,255,0.05), rgba(255,255,255,0.05) 2px, transparent 2px, transparent 4px); pointer-events:none; mix-blend-mode:overlay; opacity:.4;}
@media (pointer: coarse) { .touch-hint{display:inline; color:var(--pong-muted); font-size:.9rem} }
  ...
  // --- HUD updater (define early)
  function updateHUD(){
    if(state.hud){ state.hud.p1.textContent = String(state.score.p1); state.hud.p2.textContent = String(state.score.p2); }
  }
  ...
  function beep(freq=440, len=0.06, type='sine', gain=0.08){
    ...
    g.gain.setValueAtTime(0,t); g.gain.linearRampToValueAtTime(gain,t+0.005); g.gain.exponentialRampToValueAtTime(0.0001,t+len);
  ...
  function playReplay(){
    if(state.replay.length<10) return;
    state.paused=true;
    const frames=state.replay.slice(-Math.min(state.replay.length,5*60));
    const saveBalls=state.balls.map(b=>({...b})); const saveP1={...state.p1}, saveP2={...state.p2};
    const ctx=state.ctx; let i=0;
    const step=()=>{
  ...
  function frame(t){
    ...
    // record for replay
    if(state.recording){
      state.replay.push({p1y:state.p1?.y||0, p2y:state.p2?.y||0, balls:state.balls.map(b=>({x:b.x,y:b.y,dx:b.dx,dy:b.dy,r:b.r}))});
    ...
    if(!state.reduceMotion){
      ...
      const t2=[];
      for(const t of state.trail){ t.life-=state.dt; if(t.life>0){ c.globalAlpha=Math.max(0,Math.min(1,t.life*1.8)); circle(t.x,t.y,t.r,getCSS('--pong-accent')); t2.push(t);} }
      state.trail=t2.slice(-120); c.globalAlpha=1;
    }
    for(const b of state.balls){ circle(b.x,b.y,b.r,getCSS('--pong-fg')); }
