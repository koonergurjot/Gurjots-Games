...
// Pong v3 stable — fixes 'updateHUD is not defined' by hoisting definitively.
// Keeps responsive canvas within iframe; centering of iframe itself must be done by parent page.
(() => {
  const SLUG = "pong";
  ...
  function injectCSS() {
    ...
    style.textContent = `
:root { --pong-bg:#0b0f14; --pong-fg:#e8f1ff; --pong-accent:#69e1ff; --pong-muted:#7a8aa0; --pong-grid:#132033; --pong-glow: rgba(105,225,255,0.28); }
...
.pong-canvas-wrap{display:flex; align-items:center; justify-content:center; padding:10px; width:100%; height:100%;}
...
.theme-crt .pong-canvas::after{content:""; position:absolute; inset:0; background:repeating-linear-gradient(to bottom, rgba(255,255,255,0.05), rgba(255,255,255,0.05) 2px, transparent 2px, transparent 4px); pointer-events:none; mix-blend-mode:overlay; opacity:.4;}
@media (pointer:coarse){ .touch-hint{display:inline; color:var(--pong-muted); font-size:.9rem} }
  ...
  // Config & state
  const DFLT = { mode:"1P", ai:"Normal", toScore:11, winByTwo:true, powerups:true, sfx:true, theme:"neon", reduceMotion:false, keys:{p1Up:"KeyW", p1Down:"KeyS", p2Up:"ArrowUp", p2Down:"ArrowDown", pause:"Space"} };
  const state = { ...loadLS(), ...DFLT, running:false, debug:hasDebug(), last:0, dt:0, canvas:null, ctx:null, ratio:1, paused:false, over:false, score:{p1:0,p2:0}, balls:[], p1:null, p2:null, hud:null, diag:null, loopId:0, particles:[], shakes:0, trail:[], trailMax:20, replay:[], replayMax:5*60, recording:true, gridPhase:0 };
  ...
  function saveLS(){ const o={mode:state.mode, ai:state.ai, toScore:state.toScore, winByTwo:state.winByTwo, powerups:state.powerups, sfx:state.sfx, theme:state.theme, reduceMotion:state.reduceMotion, keys:state.keys}; try{ localStorage.setItem(LS_KEY, JSON.stringify(o)); }catch{} }

  // --- define updateHUD EARLY to avoid any scoping surprises
  ...
  function circle(x,y,r,c){ const ctx=state.ctx; ctx.fillStyle=c; ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fill(); }
  function rect(x,y,w,h,c){ const ctx=state.ctx; ctx.fillStyle=c; ctx.fillRect(x,y,w,h); }
  ...
  // Rendering
  function clear(){
    const c=state.ctx; state.gridPhase += state.dt*0.5;
    const grad=c.createLinearGradient(0,0,W,H); grad.addColorStop(0,'rgba(255,255,255,0.02)'); grad.addColorStop(1,'rgba(255,255,255,0.06)'); c.fillStyle=grad; c.fillRect(0,0,W,H);
    c.save(); c.globalAlpha=0.18; const cell=40; c.translate((state.gridPhase*20)%cell,(state.gridPhase*14)%cell);
    c.beginPath(); for(let x=-cell;x<=W+cell;x+=cell){ c.moveTo(x,0); c.lineTo(x,H);} for(let y=-cell;y<=H+cell;y+=cell){ c.moveTo(0,y); c.lineTo(W,y);} c.strokeStyle=getCSS('--pong-grid'); c.lineWidth=1; c.stroke(); c.restore();
  }
  ...
  // Game objects
  function reset(){ state.score.p1=0; state.score.p2=0; updateHUD(); state.balls.length=0; state.p1={x:32,y:H/2-60,w:18,h:120,dy:0,speed:560,maxH:180,minH:80}; state.p2={x:W-50,y:H/2-60,w:18,h:120,dy:0,speed:560,maxH:180,minH:80}; spawnBall(Math.random()<0.5?-1:1); state.over=false; state.paused=false; }
  ...
  function checkPowerupCollisions(){ for(let i=powerups.length-1;i>=0;i--){ const pu=powerups[i]; for(const b of state.balls){ const d=Math.hypot(b.x-pu.x,b.y-pu.y); if(d < b.r+pu.r+2){ const who=b.lastHit || (b.dx>0?'p2':'p1'); applyPowerup(pu.kind, who); addParticles(pu.x,pu.y,getCSS('--pong-accent'),20,260); beep(880,0.08,'sawtooth',0.1); powerups.splice(i,1); break; } } } }
  ...
  // Replay & diag
  function playReplay(){ if(state.replay.length<10) return; state.paused=true; const frames=state.replay.slice(-Math.min(state.replay.length,5*60)); const saveBalls=state.balls.map(b=>({...b})); const saveP1={...state.p1}, saveP2={...state.p2}; const ctx=state.ctx; let i=0; const step=()=>{ if(i>=frames.length){ state.p1=saveP1; state.p2=saveP2; state.balls=saveBalls; state.paused=false; return; } const f=frames[i++]; state.p1.y=f.p1y; state.p2.y=f.p2y; state.balls=f.balls.map(b=>({...b, spin:0, lastHit:null})); ctx.save(); clear(); drawNet(); rect(state.p1.x,state.p1.y,state.p1.w,state.p1.h,getCSS('--pong-fg')); rect(state.p2.x,state.p2.y,state.p2.w,state.p2.h,getCSS('--pong-fg')); for(const b of state.balls){ circle(b.x,b.y,b.r,getCSS('--pong-fg')); } ctx.restore(); requestAnimationFrame(step); }; requestAnimationFrame(step); }
  ...
  // Loop
  function frame(t){
    state.loopId=requestAnimationFrame(frame); state.dt=Math.min(0.033,(t-(state.last||t))/1000); state.last=t; if(!state.running || state.paused) return;
    updatePaddle(state.p1, state.dt); if(state.mode==='2P') updatePaddle(state.p2, state.dt); else moveAI(state.dt);
    maybeSpawnPowerup(state.dt); updatePowerups(state.dt); for(const b of state.balls){ updateBall(b, state.dt); } updateParticles(state.dt); checkPowerupCollisions();
    ...
    for(const b of state.balls){ circle(b.x,b.y,b.r,getCSS('--pong-fg')); } drawPowerups(); drawParticles(); endShake(); c.restore();
  }

  // Boot
  function start(){
    const root=ensureRoot(); injectCSS(); buildUI(root); ensureContext();
  ...
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', () => window.boot(), {once:true}); else window.boot();
})();
